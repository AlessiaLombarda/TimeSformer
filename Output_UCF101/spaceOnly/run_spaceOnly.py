...
for el in test:
    f_test.write(el+"\n")
shutil.move("test.csv", "UCF-101_preprocessed/test.csv")
i=1
for train_index, val_index in kf.split(X):
  ...
  OUTPUT_DIR "output"')
    ...
    os.chdir('..')
    #os.remove("UCF-101_preprocessed/train.csv")
    #os.remove("UCF-101_preprocessed/val.csv")

    name = "TimeSformer/o"+str(i)
    os.rename("TimeSformer/output", name)
    shutil.move(name, "output")

    name = "UCF-101_preprocessed/train"+str(i)+".csv"
    os.rename("UCF-101_preprocessed/train.csv", name)
    shutil.move(name, "folds")

    name = "UCF-101_preprocessed/val"+str(i)+".csv"
    os.rename("UCF-101_preprocessed/val.csv", name)
    shutil.move(name, "folds")

    i+=1

shutil.move("UCF-101_preprocessed/test.csv", "folds")
